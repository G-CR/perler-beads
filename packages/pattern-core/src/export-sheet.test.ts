...
test('renderExportSheet excludes blank slot from exported stats', async () => {
...
test('renderExportSheet keeps background slot stats when palette slot 0 is background', async () => {
  const buffer = await renderExportSheet({
    width: 2,
    height: 2,
    cells: [0, 1, 2, 0],
